...
def make_mouse():
    ...
    x_mouse = random.randrange(9,500,25)
    y_mouse = random.randrange(4,200,25)
    canvas.delete(mouse)  
    mouse = canvas.create_rectangle(x_mouse,y_mouse,x_mouse+25,y_mouse+25,fill='black')
...
def make_snake():
    ...
    for i in range(2):
        x_snake+=25
        listsq.append(canvas.create_rectangle(x_snake,y_snake,x_snake+25,y_snake+25,fill='blue'))

def crash():
    global x_snake,y_snake

    for i in range(0,len(listpos)-1):
        if (x_snake+25) in range(listpos[i][0],listpos[i][0]+26) and (y_snake+25) in range(listpos[i][1],listpos[i][1]+51):
            canvas.delete("all")
...
def update(x1,y1):
    ...
    if (x1+25) in range(x_mouse,x_mouse+51) and (y1+25) in range(y_mouse,y_mouse+51):        
            listsq.append(canvas.create_rectangle(x1,y1,x1+50,y1+50,fill='blue'))
            make_mouse()
    else:
